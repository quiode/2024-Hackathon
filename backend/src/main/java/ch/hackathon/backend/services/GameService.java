...
public class GameService {
    ...
    public Optional<Game> createGame(Lecture lecture, Professor professor, LectureTimeframe timeframe) {
        ...
        Game game = new Game(null, lecture, professor, timeframe, new HashSet<>(), new HashSet<>(), new HashSet<>(cardMatrix), BINGO_SIZE, BINGO_SIZE);
    ...
    public void clickCard(User user, Integer pos) {
        ...
        if(endTime.isAfter(Instant.now())) {
            //Reset valEv.
            valEv = new ValidationEvent(null, card, VALIDATION_PERCENT_NEEDED, VALIDATION_TIME_SECONDS, Instant.now(), new HashSet<>());
